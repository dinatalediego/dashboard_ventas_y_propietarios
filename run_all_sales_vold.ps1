...
<#     $DashboardPath = Join-Path $PSScriptRoot "ventas_dashboard\app_ventas_dashboard.py"
    ...
    Start-Process powershell -ArgumentList @(
     ...
     #$DashboardPath = Join-Path $PSScriptRoot "ventas_dashboard\run_app_ventas_dashboard.py"
     $DashboardPath = Join-Path $PSScriptRoot "app_ventas_dashboard.py"

    <# if (-not (Test-Path $DashboardPath)) {
        Write-Host "No existe el archivo Streamlit en:" $DashboardPath -ForegroundColor Red
        Write-Host "Archivos disponibles en ventas_dashboard:" -ForegroundColor Yellow
        Get-ChildItem (Join-Path $PSScriptRoot "ventas_dashboard") | Select-Object Name
        throw "No se encontró el dashboard" #>
    }
